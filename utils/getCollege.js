...
async function fetchCollegeDataFromAPI(collegeName) {
    try {
      ...
      const apiUrl = `https://api.data.gov/ed/collegescorecard/v1/schools?school.name=${encodeURIComponent(collegeName)}&api_key=${apiKey}&_fields=school.name,school.city,school.state,admissions.admission_rate.overall,cost.tuition.in_state`;
      ...
      if (results && results.length > 0) {
        ...
        collegeSize = schoolData.latest?.student?.size || "N/A";
      }
      ...
      return {
        ...
        acceptance_rate: acceptanceRate ? (acceptanceRate * 100).toFixed(2) + "%" : "N/A",
        location: location,
        tuition: tuition || "N/A",
  ...
  async function fetchWikipediaImage(collegeName) {
    try {
      const wikiSearchUrl = `https://en.wikipedia.org/w/api.php?action=query&format=json&titles=${encodeURIComponent(collegeName)}&prop=pageimages&pithumbsize=800`;
...
module.exports = { getCachedCollegeData }